 ...
 *
 * Fills use color-mix against the release canvas base (#0f1324) so the
 * translucent /25 look on dark navy is locked as an opaque chip — same
 * colour identity on artwork-derived atmospheres (no washout).
 */
...
export const RELEASE_COMING_SOON_PILL_CLASS =
  "bg-[color-mix(in_srgb,#f59e0b_25%,#0f1324)] text-white ring-1 ring-inset ring-amber-400/35" as const;

/** Upcoming — indigo/blue family. Must NOT use accent/turquoise. */
export const RELEASE_UPCOMING_PILL_CLASS =
  "bg-[color-mix(in_srgb,#6366f1_25%,#0f1324)] text-white ring-1 ring-inset ring-indigo-400/35" as const;
...
export const RELEASE_RELEASED_PILL_CLASS =
  "bg-[color-mix(in_srgb,#22c55e_25%,#0f1324)] text-white ring-1 ring-inset ring-green-400/35" as const;

/** Paused — neutral slate; quieter than active statuses. */
export const RELEASE_PAUSED_PILL_CLASS =
  "bg-[color-mix(in_srgb,#64748b_25%,#0f1324)] text-white ring-1 ring-inset ring-slate-400/30" as const;
